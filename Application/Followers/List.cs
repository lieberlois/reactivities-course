                    ...
                    case "followers":
                        {
                            ...
                            userFollowings.ForEach(async follower => profiles.Add(await _profileReader.ReadProfile(follower.Observer.UserName)));
                            break;
                        }
                    case "following":
                        {
                            userFollowings = await queryable.Where(x => x.Observer.UserName == request.Username).ToListAsync();
                            userFollowings.ForEach(async follower => profiles.Add(await _profileReader.ReadProfile(follower.Target.UserName)));
                            break;
                        }
                }
